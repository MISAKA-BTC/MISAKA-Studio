...
//! `kaspa-consensus-core` would drag the misakas node's entire dependency graph — RocksDB, the
//! P2P stack, ML-DSA — into a desktop app's build, on three platforms, for five hashes. What keeps the copy
...
mod tests {
    ...
    // digest_size=64)` — over the pins in the misakas node's `consensus/core/src/vlt.rs`. That is
    // what makes this a cross-check: if this module and that one had been compared to each other, a
