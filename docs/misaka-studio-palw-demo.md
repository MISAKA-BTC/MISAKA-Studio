...
* The genesis bond card was minted with `palw-rc-genesis`, the same tool and the same §1
  procedure as the misakas repository's `docs/palw-rc-testnet11-launch-runbook.md` — six ML-DSA-87 bonds, each backed
...
Every command in this section runs in a checkout of
[misakas](https://github.com/MISAKA-BTC/misakas) — the tools, the params files and the node are
the chain's, not this repository's.
