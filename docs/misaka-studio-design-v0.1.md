...
**Status:** implemented and running (2026-08-28). This describes what is in this repository, the
...
evidence.** The MISAKA network ([misakas](https://github.com/MISAKA-BTC/misakas)) settles blocks by re-deriving work that a
...
| `…/node.rs` | The MISAKA node as a supervised child: builds the join runbook's exact flag set from `NodeSettings` (producer prerequisites checked with remedy-carrying errors), one-shot wRPC (workflow-RPC over websocket) status queries, `[palw-*]` activity and `--palw-dump-classes` parsing, SIGTERM-first shutdown. |
...
It is its own repository and its own cargo workspace, split from the node's tree
([misakas](https://github.com/MISAKA-BTC/misakas)) for the reason a desktop app's dependency graph — an HTTP server, a TLS
stack, a webview binding — has no business in the lockfile a validator builds from. What keeps
the two in step is the derivations in §4, not a shared lockfile.
...
[`kaspa_consensus_core::vlt`](https://github.com/MISAKA-BTC/misakas/blob/main/consensus/core/src/vlt.rs)'s: same keyed
BLAKE2b-512, same domain keys (`misaka-vlt-model-identity-v1` and friends), same field order. A model the Studio downloads therefore already carries the `h_M` a
...
of `(class_id, artifact_root)` that [`consensus/core/src/palw_registry.rs`](https://github.com/MISAKA-BTC/misakas/blob/main/consensus/core/src/palw_registry.rs)
already has.
...
* **PALW mining, end to end** — a bonded producer configured and launched through the Studio
  ...
  misakas launch runbook's own ceremony because the build environment has no P2P route to the
  public network — while a second, independently bonded node accepted the blocks via relay and
  its panel service filed 270 `Valid` receipts on the producer's claims. The chain's
...
`BackendKind::Misaka` is reserved for the deterministic runtime the node's repository already
carries for PALW ([`misaka-palw-base0`](https://github.com/MISAKA-BTC/misakas/tree/main/misaka-palw-base0),
`misaka-palw-reference2`). That is the eventual point of the seam: a
class whose execution is adjudicable, driven from the same UI, recorded in the same shape.

## 8. Licensing

The Studio is ISC, the same licence as the MISAKA node ([misakas](https://github.com/MISAKA-BTC/misakas)). Jan (Apache-2.0,
Menlo Research) is the reference implementation it was designed against; the debt is documented in `NOTICE`
...
* **The public-testnet join is unexercised.** The producer flags the Studio builds are the join
  runbook's flags unchanged and they mined a locally minted chain (§5a), but no session from the
  build environment has connected to the live testnet-11 — it has no P2P egress. First run on a
  machine with an open network is the remaining step, and it is a configuration change, not code.
